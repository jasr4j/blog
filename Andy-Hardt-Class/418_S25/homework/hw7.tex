...
\title{Math 418, Spring 2025 -- Homework 7}
\date{}
...
\textbf{Due:} Wednesday, April 2nd, at 9:00am via Gradescope.
...
\item[1.] \textbf{Dummit and Foote \#14.3.3:} \textit{Prove that an algebraically closed field must be infinite.}

\item[2.] \textbf{Dummit and Foote \#14.3.4:} \textit{Construct the finite field of 16 elements and find a generator for the multiplicative group. How many generators are there?}

\item[3.] \textbf{Dummit and Foote \#14.3.8:} \textit{Determine the splitting field of the polynomial $f(x) = x^p - x - a$ over $\F_p$ where $a\ne 0, a\in F_p$. Show explicitly that the Galois group is cyclic.}

\item[4.] \textbf{Dummit and Foote \#14.4.2:} \textit{Find a primitive element for $K := \Q(\sqrt{2}, \sqrt{3}, \sqrt{5})$ over $\Q$.}

\item[5.] \textbf{Dummit and Foote \#14.4.3:} \textit{Let $F$ be a field contained in the ring $\Mat_n(\Q)$ of $n \times n$ matrices over $\Q$. Here, $\Q\subseteq \Mat_n(\Q)$ is identified with the scalar diagonal matrices by the inclusion \[q\mapsto qI = \begin{bmatrix} q &&& \\ &q&& \\ &&\cdots& \\ &&&q\end{bmatrix}.\] Prove that $[F : \Q] \le n$.} (\emph{I do have a hint for this one, if you ask})
